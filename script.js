...
async function pickCategoryValue(current){
  const result = await swalFire({
    ...
    html: `<div class="swal-list">
        <button type="button" class="swal-list__item ${current==='income'?'swal-list__item--active':''}" data-cat="income"><span class="cat-icon cat-icon--income">${ICONS.arrowUp}</span><span>Pendapatan</span></button>
        <button type="button" class="swal-list__item ${current==='expense'?'swal-list__item--active':''}" data-cat="expense"><span class="cat-icon cat-icon--expense">${ICONS.arrowDown}</span><span>Pengeluaran</span></button>
      </div>`,
    showConfirmButton: false,
    showCancelButton: true,
    cancelButtonText: 'Batal',
